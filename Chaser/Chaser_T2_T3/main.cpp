...
public:
	...
	int Rotate = 0; // Упр. команда, 0 – двигаться прямо, -1 – поворачивать налево, 1 – поворачивать направо
public:
	...
	void SetSpeed() // Вычисление скорости через гипотенузу
	{
		V = sqrt((Vx * Vx) + (Vy * Vy));
	}
	void SetRotate(double AngleDif) // Выбор в какую сторону поворачивать
	{								// AngleDif - разница между углом преследователя и нахождением точки, в радианах
		if (abs(AngleDif) < 0.1)	// Точка прямо
		{
			Rotate = 0;
		}
		else if (AngleDif > 0)		// Точка левее
		{
			Rotate = -1;
		}
		else						// Точка правее
...
int main()
{
	...
	chaser->SetRadius();			// Радиус поворота
	...
	while (!Target::ChasedAll)
	{
		...
		// Определение поворота преследователя
		if (!Target::ChasedAll)
		{
			BestCombinationTargets[0]->CalculateAngle(BufferXChaser, BufferYChaser, chaser->V);

			double ChaserAngle = atan2(chaser->Vy, chaser->Vx);
			double AngleDif = BestCombinationTargets[0]->ChaseAngle - ChaserAngle;

			if (AngleDif > M_PI)  AngleDif -= 2 * M_PI; // Нормализуем угол, от -pi до pi
			if (AngleDif < -M_PI) AngleDif += 2 * M_PI;
			chaser->SetRotate(AngleDif);
		}

		// Вывод упр. команд
		cout << chaser->Rotate << endl;
